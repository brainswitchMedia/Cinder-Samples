...
class Blockletter02App : public App {
public:
...
Blockletter02App::Blockletter02App()
{
...
void Blockletter02App::setup()
{
    ...
    // Colors
    mColor1 = vec3( 0.26, 0.34, 0.34 );
    mColor2 = vec3( 0.74, 0.24, 0.15 );
...
void Blockletter02App::update()
{
    ...
    rotationSpeed1 = ( 1.0 - ease1 ) * ( mSceneRotationSpeed * 9.8f );

    console() << "ease1: " << ease1 << std::endl;
    console() << "rotationSpeed1: " << rotationSpeed1 << std::endl;
    ...
    mSceneRotation *= rotate( -mSceneRotationSpeed * 1.0f - rotationSpeed1, vec3( 1.0, 0.0, 0.0 ) );
...
void Blockletter02App::draw()
{
    ...
    // Render scene in 2 textures ( GL_COLOR_ATTACHMENT1 for the bloom pass )
    {
        ...
        float centeredPyramidSize = 0.53f;
        ...
        // Render centered Pyramid
        {
            ...
            modelMatrix = modelMatrix * mSceneRotation2;
...
void Blockletter02App::createPrimitivesPositionsFromCube()
{
    mPosCoords.clear();
    mIndices.clear();
...
gl::VboMeshRef Blockletter02App::createPrimitivePyramidFromPoints( vec3 point1, vec3 point2, vec3 point3, vec3 point4, bool cavity, bool setFacesSpheresPos )
{
    ...
    if ( setFacesSpheresPos == true ) setSpheresPositions( mPosCoordsPyramids, mIndicesPyramids );
...
void Blockletter02App::setSpheresPositions( const std::vector<ci::vec3> &posCoords, const std::vector<uint16_t> &indices )
{
...
vec3 Blockletter02App::computeTriangleGravitycenter( const vec3 &point1, const vec3 &point2, const vec3 &point3 )
{
    return vec3( point1.x + point2.x + point3.x, point1.y + point2.y + point3.y, point1.z + point2.z + point3.z ) / 3.0f;
}


vec3 Blockletter02App::computeTriangleNormal( const vec3 &pos1, const vec3 &pos2, const vec3 &pos3 )
{
    vec3 e0 = pos2 - pos1;
    vec3 e1 = pos3 - pos1;
...
void Blockletter02App::loadShaders()
{
    try {
...
void Blockletter02App::createGeometry()
{
...
void Blockletter02App::loadTextures()
{
    // Load the textures.
...
// Save renderFbo
void Blockletter02App::saveImage()
{
    //console() << "mCurrentFrame : " << mCurrentFrame << std::endl;
    ...
    else if (  ( mLowfpsXML == false && mSaveScreen == true  ))
    {
        // console() << "mCurrentFrame: " << mCurrentFrame << std::endl;
...
void Blockletter02App::createParams()
{
...
void Blockletter02App::setXMLAnimationPhases()
{
    if( mXMLcom.mIsBeat1 == true )
    {
        mbeatCounter_1 = 0.0f;
        mbeatCounter_2 = 0.0f;
        mIsBeat2 = true;
    ...
    mDistancePower1 = -0.2 - lin1;
    
    float circleCoef_Min = 0.0f;
    float circleCoef_Max = 0.41f;
    float lin2 = linearize( 0.0f, 1.0f, circleCoef_Min, circleCoef_Max, ease2 );
    mCircleSize = 1.35 + lin2;
    
    if( mXMLcom.mIsBeat2 == true )
    {
        mIsBeat3 = true;
        mbeatCounter_3 = 0.0f;
    }
    
    float ease3 = 1.0 - easeOutExpo( clamp( mbeatCounter_3, 0.0f, 1.0f ) );
    if ( mbeatCounter_3 < 1.0f ) mbeatCounter_3 += 0.035f;
    ...
    float distCoef_Min = 0.0f;
    float distsizeCoef_Max = 0.35f;
    float lin4 = linearize( 0.0f, 1.0f, distCoef_Min, distsizeCoef_Max, ease3 );
    mDistancePower2 = -0.1f - lin4;
    
    float brigthCoef_Min = 0.0f;
    float brigthsizeCoef_Max = 4.0f;
    float lin5 = linearize( 0.0f, 1.0f, brigthCoef_Min, brigthsizeCoef_Max, ease3 );
    mBrightness = 2.0f + lin5;
    
    float distanceConverstionTetraedre2_Min = 0.0f;
    float distanceConverstionTetraedre2_Max = 0.35f;
    float lin6 = linearize( 0.0f, 1.0f, distanceConverstionTetraedre2_Min, distanceConverstionTetraedre2_Max, ease3 );
    mDistanceConverstionTetraedre2 = 10.45f - lin6;
}


void Blockletter02App::transition_0( bool start )
{
    if ( start == true )
    {
    }
...
void Blockletter02App::transition_1( bool start )
{
    if ( start == true )
    {
    }
...
void Blockletter02App::transition_2( bool start )
{
    if ( start == true )
    {
    }
...
void Blockletter02App::mouseDown( MouseEvent event )
{
    //mIsBeat = true;
    mIsBeat2 = true;
    mIsBeat3 = true;
...
void Blockletter02App::mouseDrag( MouseEvent event )
{
    
}


void Blockletter02App::keyDown( KeyEvent event )
{
...
float Blockletter02App::linearize( float a0, float a1, float b0, float b1, float in )
{
    return ( ( in - a0 ) / ( a1 - a0 ) ) * ( b1 - b0 ) + b0;
}


vec3 Blockletter02App::rotateAxis( bool beat, vec3 &rotationVectorFrom, vec3 &rotationVectorTo, float easeSpeed )
{
    if ( beat == true )
    {
...
// Vector slerp
vec3 Blockletter02App::slerp( vec3 &start, vec3 &end, float percent )
{
...
mat4 Blockletter02App::rotationAlign( vec3 d, vec3 z )
{
    vec3  v = cross( z, d );
    float c = dot( z, d );
    float k = 1.0/(1.0+c);
...
CINDER_APP( Blockletter02App, RendererGl( RendererGl::Options() ), prepareSettings )
