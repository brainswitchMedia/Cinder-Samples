...
ReactiveCam::ReactiveCam()
{
    ...
    mCamParams.mAxisXRotationSpeed  = 0.25f;
    mCamParams.mAxisYRotationSpeed  = -0.25f;
    mCamParams.mRotationSpeedCoef   = 0.00f;
    //mCamParams.mRotationSpeed       = 0.005f; // 30fps
    mCamParams.mRotationSpeed       = 0.005f;  // 60fps
    mbeatCounter                    = 0.0f;
    ...
    quat mInitialQuat;
    mChangeRotation                 = 0;
    mInit_cam_eye                   = vec3( 1.0f, 1.0f, -1.0f );
...
void ReactiveCam::set( float cameraDistance, vec3 lookAt, float min, float max )
{
    ...
    mCam.lookAt( normalize(mInit_cam_eye), lookAt, vec3( 0.0f, 0.0f, 0.0f ) );
...
vec3 ReactiveCam::getViewDirection()
{
...
void ReactiveCam::update( bool beat, vec3 rotationVector, bool isEase, float rotationSpeed  )
{
    ...
    if ( isEase == true )
    {
        ...
        rotation = ( 1.0 - ease) * (mCamParams.mRotationSpeedCoef + rotationSpeed );
    }

    if ( mbeatCounter < 1.0f ) mbeatCounter += 0.02;
