...
void main()
{
    vec3 spectrumCol_Mode1 = texture( uTexSpectrum_Mode1, vec2( uRedCoef * vec3( vSize ).r, 0.0 ) ).rgb;
    vec3 spectrumCol_Mode2 = texture( uTexSpectrum_Mode2, vec2( uRedCoef * vec3( vSize ).r, 0.0 ) ).rgb;
    ...
    float DepthLigth = -( uWorldtoLightMatrix * vVertexWorldSpace ).z;
    ...
    // 0 - 200  ->  0 - 1
    float distToLigth = length( uWorldtoLightMatrix * vVertexWorldSpace );
    ...
    vec3 L = normalize( uLightPosition - vVertexWorldSpace.xyz );
    vec3 H = normalize( V + L );
    float distance    = length( uLightPosition - vVertexWorldSpace.xyz );
    ...
    if ( uMode == 0 )
    {
        oFragColor0.rgb = grad2.r * spectrumCol_Mode1 * clamp(translulencyDist2, 0, 1 ) + grad2.r * clamp( specular, vec3(0), vec3(1) ) * clamp(translulencyDist2, 0, 1);
    ...
    oFragColor0.rgb *= uColorIndex;
    oFragColor1.rgb *= uColorIndex;
    oFragColor2.rgb *= uColorIndex;
