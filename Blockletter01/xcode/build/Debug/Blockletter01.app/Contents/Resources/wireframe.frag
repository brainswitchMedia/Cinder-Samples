...
void main()
{
    ...
    vec3 finalColor = clamp( translulencyDist, 0.0, 1.0 ) * ( 17.0 * colorMix + uColor * pow( ( 1.0 - sin( uCircleSize * distToCenterLight ) ), uPower1 )  ) + 12.0 * uColor  * specular;

    //vec3 finalColor = clamp( translulencyDist, 0.0, 1.0 ) * ( 20.0 * colorMix + uColor * pow( ( 1 - distToCenterLight ), uPower1 ) + uColor * vec3(12.0) * specular  );
    if ( uFront == true )
    {
        oFragColor0.rgb = clamp( finalColor, -20.0, 20.0 );
    }
    else
    {
        oFragColor0.rgb = clamp( finalColor, -15.0, 15.0 );
    }
    //oFragColor0.rgb = vec3( specular) ;

    //oFragColor1.rgb = finalColorTrails;
    oFragColor1.rgb = finalColorTrails;
