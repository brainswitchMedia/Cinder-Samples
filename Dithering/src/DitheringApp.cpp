...
public:
    ...
    gl::BatchRef                mPrimitive[2], mBatchFxaaRect, mBatchUpscaleRect, mBatchDitherRect, mBatchToScreenRect;
    gl::GlslProgRef             mIlluminationShader, mFxaaShader, mDitheringShader, mPixelArtUpscalingShader;
...
void DitheringApp::prepareSettings( Settings *settings )
{
...
void DitheringApp::setup()
{
    ...
    mCamera.lookAt( vec3( 3, 3, 3 ), vec3( 0, 0, 0 ) );
...
gl::VboMeshRef DitheringApp::createPrimitiveFromCube( bool dual )
{
    mPosCoords.clear();
    mIndices.clear();
    mNormals.clear();
...
void DitheringApp::mouseDown( MouseEvent event )
{
}


void DitheringApp::update()
{
...
void DitheringApp::draw()
{
    ...
    // Dithering
    {
        ...
        mBatchDitherRect->getGlslProg()->uniform( "uTime", 10.0f * (float)getElapsedSeconds() );
...
void DitheringApp::createGeometry()
{
...
void DitheringApp::loadTextures()
{
    // Load the textures.
    gl::Texture::Format fmt;
    fmt.setAutoInternalFormat();
    fmt.setWrap( GL_REPEAT, GL_REPEAT );
    mTextureBlurNoise = gl::Texture::create( loadImage( loadAsset( "textures/mask_1920_1080.png" ) ), fmt );
}


// Save renderFbo
void DitheringApp::saveImage()
{
    // Pull down the current window as a surface and pass it to writeImage
    writeImage( getHomeDirectory() / "deformedSphere" / ( toString(1) + "_" + toString( mCurrentFrame ) + ".png" ), mPixelArtUpscaleFBO->getTexture2d( GL_COLOR_ATTACHMENT0 )->createSource(), ImageTarget::Options().quality(1.0f) );
}


CINDER_APP( DitheringApp, RendererGl, &DitheringApp::prepareSettings )
