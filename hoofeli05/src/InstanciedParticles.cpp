...
void InstanciedParticles::setup( CameraPersp *cameraPersp, CameraPersp *lightCameraPersp, const vector<SphereParticle> &particles, uint8_t meshType )
{
    ...
    for( int i = 0; i < particlesSize; i++ ){
        ...
        p.SphereType = particles[i].mParticleType;
    ...
    instanceDataLayout.append( geom::Attrib::CUSTOM_3, 1, sizeof( InstanceData ), offsetof( InstanceData, type ), 1 /* per instance*/ );
    ...
    mapping[geom::Attrib::CUSTOM_3] = "aType";
...
void InstanciedParticles::update( float sizeCoef0, float sizeCoef1, mat4 &sceneRotation, vec2 fboSize, vec3 cameraPosition, bool translate )
{
    ...
    for( vector<Particle>::iterator p = mParticles.begin(); p != mParticles.end(); ){
        ...
        // Translate
        float sizeParticle = 0.0f;
        
        if( p->SphereType == 0 )
        {
            sizeParticle = sizeCoef0 * p->DistToFaceGravityCenter;
            
            if ( translate == true )
            {
                modelMatrix = glm::translate( modelMatrix, 0.01f * sizeCoef0 * p->Normal );
            }
        }
        
        else if( p->SphereType == 1 )
        {
            sizeParticle = sizeCoef1;
            
            modelMatrix = glm::translate( modelMatrix, 0.01f * sizeCoef1 * p->Normal );
            //modelMatrix = glm::translate( modelMatrix, 0.03f * p->Normal );
        }
...
void InstanciedParticles::loadTextures()
{
    ...
    mTextureStripes = gl::Texture::create( loadImage( loadAsset( "textures/stripes0.jpg" ) ), fmt );
}
