...
void TubeManager::setup( const std::string &pathVert, const std::string &pathFrag, const std::string &pathTexture1, const std::string &pathTexture2 )
{
    ...
    mTubesParams.mMaxScale1             = 1.1f;
    mTubesParams.mLengthParticleLoc     = 50;
    mTubesParams.mTubeLength            = 60;
    mTubesParams.mVelCoef               = 0.08f;
    mTubesParams.mVelDamping            = 0.94f;
...
void TubeManager::update( vector <ci::vec3> &alocParticleEmitters )
{
    // Number of tubes
    mNumberOfTubes = mTubes.size();
    ...
    if ( mNumberOfTubes != 0 )
    {
        ...
        for( int i = 0; i < tubeToUpdate; i++ )
        {
            ...
            if ( mTubes[i]->mScale1 < mTubesParams.mMaxScale1 * mTubes[i]->mRandom && mTubesParams.mAutoGrowDiam == true )
            {
                mTubes[i]->mScale1 += 0.1f;
            }
            else
            {
