...
void main( void )
{
	...
	vec3 rgbUL	= texture( uSampler, vec2( TexCoord0.x, TexCoord0.y ) + vec2( -1.0, -1.0 ) * uPixel ).xyz;
	vec3 rgbUR	= texture( uSampler, vec2( TexCoord0.x, TexCoord0.y ) + vec2(  1.0, -1.0 ) * uPixel ).xyz;
	vec3 rgbBL	= texture( uSampler, vec2( TexCoord0.x, TexCoord0.y ) + vec2( -1.0,  1.0 ) * uPixel ).xyz;
	vec3 rgbBR	= texture( uSampler, vec2( TexCoord0.x, TexCoord0.y ) + vec2(  1.0,  1.0 ) * uPixel ).xyz;
	vec3 rgbM	= texture( uSampler, vec2( TexCoord0.x, TexCoord0.y ) ).xyz;
	...
	vec3 color0 = 0.5 * (
		texture( uSampler, vec2( TexCoord0.x, TexCoord0.y ) + dir * ( 1.0 / 3.0 - 0.5 ) ).rgb +
		texture( uSampler, vec2( TexCoord0.x, TexCoord0.y ) + dir * ( 2.0 / 3.0 - 0.5 ) ).rgb );
	vec3 color1 = color0 * 0.5 + 0.25 * (
		texture( uSampler, vec2( TexCoord0.x, TexCoord0.y ) + dir * -0.5 ).rgb +
		texture( uSampler, vec2( TexCoord0.x, TexCoord0.y ) + dir *  0.5 ).rgb );
