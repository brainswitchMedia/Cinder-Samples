...
{        
    if ( mTubeIsBuilded )
    {
        ...
        // to test vertex position
        mVertexOrder.clear();
        mtesttab1.clear();
        mtesttab2.clear();
        
        // memorisation du premier point de la premiere table des frames
        /*Vec3f first = ( mFrames[0]*( mProf[0] * 100.0f ) );
        mNewVertex = first;
        
        //console() << first << std::endl;
        // initialisation si mFirstVertex n'a pas encore ete renseigne
        if ( mFirstVertex == Vec3f( 0.0f, 0.0f, 0.0f ) ) mFirstVertex = mFirstVertexTest = first;
        
        //mTestVector1 = ( mFrames[0]*( mProf[0] * 20.0f ) - mFrames[0] * ( mProf[0] * 10.0f ) ).normalized(); //if ( ci == 0 && i ==0 )
        
        //mLastFrame = ( mFrames[0]*( mProf[0] ) ); //if ( ci == 0 && i ==0 )
        //mTestVector2 = rotation*mTestVector1;
        
        mTestVector3 = mTestVector2 = mTestVector1;
        mTestVector1 = origine;
        
        float dotProduct1 = mTestVector2.dot( mTestVector1 );
        float lenght11 = mTestVector2.length();
        float lenght21 = mTestVector1.length();
        float cos1 = dotProduct1 / (lenght11 * lenght21);
        float clamp1 = cos1 <= -1.0f ? -1.0f : cos1 >= 1.0f ? 1.0f : cos1; // clamp parce que cos n'est defini qu'entre -1 et 1.
        float angle1 = acos( clamp1 );
        
        //console() << mTestVector1 << std::endl;
        //console() << mFirstVertex << std::endl;
        
        Vec3f rotated = Quatf( cross( mTestVector1, mTestVector2 ), angle1 ) * mTestVector1;

        mRotatedVertex = Quatf( cross( mTestVector2, mTestVector1 ), angle1 ) * mFirstVertex;
        
        //console() << angle1 * 180 / 3.14159265359f << std::endl;
        //console() << angle1 << std::endl;
         
        
        
        if ( mTestVector2 != Vec3f( 0.0f, 0.0f, 0.0f ) )
        {
            mTestVector2 = rotated;
            mFirstVertex = mNewVertex;
        }
        else console() << "error vector not initialised" << std::endl;
        */
        
        // memorisation du premier point de la premiere table des frames
        //Vec3f first = mFrames[0]*( mProf[0]* ( mScale1 / (float)( mLength - 1 ) ) );
        
        // initialisation si mFirstVertex n'a pas encore ete renseigne
        //if ( mFirstVertex == Vec3f( 0.0f, 0.0f, 0.0f ) ) mFirstVertex = first;
        
        // calcul du decallage du premier point entre 2 iterations de programme
        /*float dotProduct = first.dot( mFirstVertex );
        float lenght1 = first.length();
        float lenght2 = mFirstVertex.length();
        float cosi = dotProduct / (lenght1 * lenght2);
        float clamp = cosi <= -1.0f ? -1.0f : cosi >= 1.0f ? 1.0f : cosi; // clamp parce que cos n'est defini qu'entre -1 et 1.
        float angle = acos( clamp );
        
        // si il y a un decallage superieur a 1 fraction ( disque divise en mProf.size() fractions )
        if ( abs( round( ( angle * 180 / 3.14159265359f ) * mProf.size() / 360 ) ) > 1 )
        {
            // on inverse la variable memoire du decallage (il n'y a que 2 cas, initial et oppose) et on memorise la nouvelle premiere position
            mRotateVertex = !mRotateVertex;
            mFirstVertex = first;
        }
        
        // si il y a un decallage par rapport a l'ordre initial on reorganise l'ordre des points pour que l'ordre des vertex soit toujours
        // identique a l'odre initial (creation du VBO)
        if ( mRotateVertex )
        {
            for( int i = 0; i < mProf.size(); ++i )
            {
                mVertexOrder.push_back(i);
            }
            rotate( mVertexOrder.begin(), mVertexOrder.begin() + round( mProf.size()/2 ), mVertexOrder.end() );
        }*/
        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        for( int i = 0; i < mLength; ++i )
        {
            ...
            //float r0 = sin( (float)( i ) / (float)( mLength - 1 ) * 3.141592f );
            ...
            for( int ci = 0; ci < profileSize; ++ci )
            {
                ...
                /*if ( i == 0 )
                {
                    mtesttab1.push_back(mat0 * ( mProf[idx0] * rs0) );
                }
                if ( i == 1 )
                {
                    mtesttab2.push_back(mat0 * ( mProf[idx] * rs0) );
                }*/
...
void Tube::drawTangents( int lenght ) {
    ...
    {
        int k = i % 2;
        ( k == 0 ) ? gl::color( Color(0, 1, 1) ) : gl::color( Color(1, 1, 0) );
        gl::drawVector( mPosCoords[i], mPosCoords[i] + mNormals[i]*4.0f, 0.01, 0.002f );
    }*/
    
    // To test vertex position
     /*glLineWidth( 8.0f );
     glBegin( GL_LINE_STRIP );
     for( int i = 0; i < ( mtesttab1.size() ); ++i )
     {
         gl::color( Color( 1.0f, 0.1f*i, 0.0f ) );
         glVertex3f( mtesttab1[i] );
     }
     glEnd();
     
     gl::color( Color( 0.0f, 1.0f, 0.0f ) );
     glBegin( GL_LINE_STRIP );
     for( int i = 0; i < ( mtesttab2.size() ); ++i )
     {
         glVertex3f( mtesttab2[i] );
     }
     glEnd();*/
    glLineWidth( 2.0f );
    /*
    ...
    glBegin( GL_LINE_STRIP );
    glVertex3f( Vec3f(0.0f, 0.0f, 0.0f) );
    glVertex3f( mTestVector1 );
    glEnd();*/
   /* gl::color( Color( 1.0f, 0.0f, 1.0f ) );
    ...
    glBegin( GL_LINE_STRIP );
    glVertex3f( Vec3f(0.0f, 0.0f, 0.0f) );
    glVertex3f( mTestVector2 );
    glEnd();
    gl::color( Color( 1.0f, 1.0f, 0.0f ) );
    glBegin( GL_LINE_STRIP );
    glVertex3f( Vec3f(0.0f, 0.0f, 0.0f) );
    glVertex3f( mTestVector3 );
    glEnd();
    gl::color( Color( 1.0f, 1.0f, 1.0f ) );
    glBegin( GL_LINE_STRIP );
    glVertex3f( mTestVector3 );
    glVertex3f( mNewVertex );
    glEnd();
    glLineWidth( 4.0f );
    gl::color( Color( 1.0f, 0.0f, 0.0f ) );
    glBegin( GL_LINE_STRIP );
    glVertex3f( mTestVector3 );
    glVertex3f( mRotatedVertex );
    glEnd();*/

    /*gl::color( Color( 0.0f, 1.0f, 1.0f ) );
    glBegin( GL_LINE_STRIP );
    glVertex3f( mTestVector3+mRotatedVertex*10.0f );
    glVertex3f( mTestVector3+mRotatedVertex*10.0f+mFirstVertexTest*50.0f );
    glEnd();*/
